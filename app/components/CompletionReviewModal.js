...
export default function CompletionReviewModal({ bookingId, onComplete }) {
  ...
  const submitReview = async () => {
    ...
    if (!description.trim()) {
      setError("Please add a short description of the service.");
      return;
    }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 px-4">
      <div className="w-full max-w-md rounded-md border border-slate-200 bg-white p-5 shadow-2xl">
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-xs leading-relaxed text-slate-500">
              How was your service experience with this technician?
            </p>
          </div>
          <button
            type="button"
        ...
        <textarea
          ...
          maxLength={2000}
          rows={4}
          placeholder="Write a short review about the service quality and professional conduct..."
