    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 px-4">
      <div className="w-full max-w-md rounded-md border border-slate-200 bg-white p-5 shadow-2xl">
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <h2 className="mt-1 text-base font-bold tracking-tight text-slate-900">
              {isParticipantStage ? "Rate your worker" : "Rate Kazilen"}
            </h2>
            <p className="mt-1 text-xs leading-relaxed text-slate-500">
              {isParticipantStage
                ? "How was your service experience with this worker?"
        ...
        <label className="mt-5 block text-xs font-bold text-slate-700" htmlFor="review-description">
          {isParticipantStage ? "Description about the worker" : "Platform feedback"}
        </label>
        <textarea
          ...
          maxLength={2000}
          rows={4}
          placeholder={isParticipantStage ? "Write a short review about the service..." : "Write a short note about Kazilen..."}
