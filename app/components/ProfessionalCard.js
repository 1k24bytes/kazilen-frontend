...
export default function ProfessionalCard({ professional, subCategory }) {
	...
	// Look up subcategory pricing dynamically from services.json
	const foundSubCategory = servicesData?.subCategories?.find(
		(s) => s.id === subCategory
	);

	const price = foundSubCategory?.price || professional.price || 150;
	...
	return (
		<div className="w-full bg-white rounded-2xl border border-slate-200 p-5 shadow-2xs hover:border-[#ff8a4c]/50 transition flex flex-col justify-between space-y-4">
			<div className="space-y-3">
				...
				<div className="flex items-start justify-between gap-3">
					...
					<div className="inline-flex items-center gap-1 px-2.5 py-1 rounded-xl bg-amber-50 border border-amber-200 text-amber-800 text-xs font-bold shrink-0">
			...
			<div className="pt-4 border-t border-slate-100 flex items-center justify-between gap-3">
				<ViewDetailsButton
					...
					details={`Fixed rate for ${serviceLabel}`}
					price={price}
				/>
				...
				<div className="flex items-center gap-3">
					...
					<button
						...
						className="px-4 py-2 text-xs font-bold rounded-xl bg-[#ff8a4c] hover:bg-[#f07432] text-white shadow-2xs transition cursor-pointer active:scale-98"
					>
						Book Now
					</button>
				</div>
