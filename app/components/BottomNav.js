...
export default function BottomNav() {
  ...
  return (
    <nav aria-label="Customer Navigation" className="fixed bottom-0 inset-x-0 z-40 bg-white/95 backdrop-blur-md border-t border-slate-200 shadow-md">
      <div className="max-w-4xl mx-auto px-2 sm:px-6">
        <div className="flex items-center justify-around">
          {NAV_ITEMS.map((item) => {
            ...
            if (item.href === '/') {
              isActive = pathname === '/';
            } else if (item.href === '/profile/orders') {
              isActive = pathname === '/profile/orders' || pathname.startsWith('/profile/orders/');
            } else if (item.href === '/profile/addresses') {
              isActive = pathname === '/profile/addresses' || pathname === '/select-address';
